...
-- 1. After Like: log action and generate recommendations
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN NULL; -- procedure may not exist at schema creation time
    END;
...
-- 2. Before Rating: validate value is 1-5
...
-- 3. After Rating: log action and regenerate recommendations
...
-- 4. After ListItem insert: log the action
--    (AuditLog is written HERE by the trigger; the procedure does NOT write it)
...
-- ====================================================================
-- STORED PROCEDURES
-- (Full definitions live in db/AddToList.sql and db/BulkOperations.sql.
--  Only the calculate_media_score function is defined inline here.)
-- ====================================================================
...
INSERT INTO MediaType (type_name) VALUES ('movie'), ('book'), ('music')
...
INSERT INTO Genre (genre_name) VALUES
    ...
    ('Science Fiction'), ('Thriller'), ('War'), ('Western'), ('Fiction'),
    ('Non-Fiction'), ('Literature'), ('Poetry'), ('Philosophy'),
    ('Psychology'), ('Self-Help'), ('Technology'), ('Business'), ('Art'), ('Sport')
ON CONFLICT (genre_name) DO NOTHING;

-- NOTE: The default admin password below is a bcrypt hash of 'admin123'.
-- Replace it after first login using the application's account settings.
INSERT INTO Users (name, email, password_hash, role) VALUES
    ('Admin User', 'admin@vault.com',
     '$2b$12$placeholderHashReplaceThisBeforeDeployment', 'admin')
...
BEGIN
    ...
    RAISE NOTICE 'Run db/AddToList.sql, db/generate_recommendations.sql,';
    RAISE NOTICE 'db/BulkOperations.sql, db/AnalyticsViews.sql,';
    RAISE NOTICE 'db/addMediaTag.sql, db/get_similar_media.sql,';
    RAISE NOTICE 'db/compute_user_themes.sql, db/LinkMediaGenre.sql,';
    RAISE NOTICE 'db/MediaSearchGallery.sql, db/SubmitUserRatings.sql';
    RAISE NOTICE 'to complete the system.';
END $$;